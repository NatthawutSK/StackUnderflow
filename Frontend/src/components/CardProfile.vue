        ...
        <v-col>
          <v-card class="mx-auto mb-6 d-flex pa-5" rounded="0">
            ...
            <div>
              ...
              {{
                !!userStore.profiledata.follower?.find(
                  ({ mem_id }) => {
                    return mem_id == userStore.user.mem_id;
                  }
                )
              }}
    ...
    " @input="vp$?.cfpassword?.$touch"></v-text-field>
      ...
      <v-window v-model="step"  >
        <v-window-item :value="1">
          ...
          <v-card v-if="userStore.profiledata.question > 0" class="text-decoration-none pa-5 ma-3" v-for="question in userStore.profiledata.top_question">
          <div class="d-flex justify-center align-center text-decoration-none" >
            <div    class="d-flex align-center"><v-icon  icon="mdi-thumb-up"></v-icon><p class="ms-3">{{ question.post_vote }}</p></div>
          <v-card-text>
      ...
      <p v-else>You don't have any Question</p>
        ...
        <v-window-item :value="2">
          ...
          <v-card v-if="userStore.profiledata.answer > 0" v-for="answer in userStore.profiledata.top_answer" class="pa-5 ma-3">
